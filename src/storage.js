...
// Exposes async-style get/set/remove with sync-flavored API via React state.
...
const backend = hasWindowStorage
  ? {
      ...
      remove: async (k) => {
        try { return await window.storage.remove(k); } catch { return null; }
      },
      keys: async () => {
        try { return (await window.storage.keys?.()) ?? []; } catch { return []; }
      }
    }
  : {
      get: async (k) => {
...
// Load all keys matching a prefix into an object map
export async function loadAll(prefix) {
  ...
  await Promise.all(
    all
      ...
      .map(async (k) => { out[k] = await storage.get(k); })
  );
  return out;
}
