...
export default function StudyBlock({ chapterId, block, onStateChange }) {
  const reviewedKey = `studyguide:ch${chapterId}:block${block.id}:reviewed`;
  const confKey = `studyguide:ch${chapterId}:block${block.id}:confidence`;
  ...
  return (
    <section id={`block-${block.id}`} className="scroll-mt-24 bg-white border border-slate-200 rounded-xl shadow-sm mb-6">
      ...
      <footer className="px-5 py-4 border-t border-slate-100 bg-slate-50 rounded-b-xl flex flex-wrap items-center gap-4 justify-between">
